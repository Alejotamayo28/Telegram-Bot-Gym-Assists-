...
export function getUserCredentials(userId: number): Required<UserCredentials> {
  return userState[userId].data.credentials;
}

export function getUserExercise(userId: number): Required<Exercise> {
  return userState[userId].data.exercise;
}

export function getUserFamily(userId: number): Required<Family> {
  return userState[userId].data.family;
}

export function getFamilyMember(userId: number): Required<FamilyMember> {
...
export function getUserSelectedMessagesId(userId: number): Required<Message> {
...
export const deleteUserMessage = async (ctx: Context) => {
  if (userMessageTest[ctx.from!.id]) {
    ...
    console.log(
      `User message with the id: ${userMessageTest[ctx.from!.id]} deleted`,
    );
