...
export class FamilyInlineKeyboard extends MessageTemplate {
  ...
  protected prepareMessage() {
    ...
    const keyboard: InlineKeyboardMarkup = {
      inline_keyboard: [
        [
          this.createButton(`Visualizar familias`, {
            action: FamilyInlinekeyboardAction.viewFamily,
          }),
          this.createButton(`Unirse a familia`, {
            action: FamilyInlinekeyboardAction.joinFamily,
          }),
        ],
        [
          this.createButton(`Crear familia`, {
            action: FamilyInlinekeyboardAction.createFamily,
          }),
        ],
      ],
    };
  ...
  async handleOptions(
    ...
    const handler: { [key: string]: () => Promise<void> } = {
      [FamilyInlinekeyboardAction.viewFamily]: this.handleViewFamilies.bind(
        this,
        bot,
      ),
      [FamilyInlinekeyboardAction.createFamily]: this.handleCreateFamily.bind(
        this,
        ctx,
        bot,
      ),
      [FamilyInlinekeyboardAction.joinFamily]: this.handleJoinFamily.bind(
        this,
        ctx,
        bot,
      ),
