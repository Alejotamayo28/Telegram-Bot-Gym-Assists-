...
import { ExercisePostVerificationHandler } from "./inlineKeyboard/ExercisePostInlieKeyboard";
import { ConfirmationMenuModel } from "../../../model/confirmationMenuModel";
import { BotStage, saveBotMessage, updateUserStage } from "../../../userState";
...
export const PostExerciseVerificationController = async (
  ctx: Context,
  bot: Telegraf,
) => {
  ...
  try {
    ...
    const postExerciseController = async (): Promise<void> => {
      await BotUtils.sendBotMessage(
        ctx,
        botMessages.inputRequest.prompts.postMethod.exerciseName,
      );
    ...
    await setUpKeyboardIteration(ctx, daysKeyboard, bot, {
      ...
      nextStep: async () => await postExerciseController(),
    });
  } catch (error) {
    console.error(`Error: `, error);
  }
