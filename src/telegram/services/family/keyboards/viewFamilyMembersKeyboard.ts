...
import { UserFamilyMemberResponse, updateUserState } from "../../../../userState"
import { familiesMethod, setUpKeyboardIteration, regexPattern } from "../../utils"
...
export class ViewFamilyMembersInlineKeybaordNotWorking extends MessageTemplate {
    ...
    protected prepareMessage() {
        const grouppedButtons = BuildFamilyInline.createInlineKeyboard<
            UserFamilyMemberResponse>("viewFamiliesMember", this.data)
    ...
    private options: { [key in keyof typeof familiesMethod]: (ctx: Context, bot: Telegraf,
        data?: UserFamilyMemberResponse) => Promise<void> } = {
            getMethod: async (ctx: Context, bot: Telegraf,
                data?: UserFamilyMemberResponse): Promise<void> => {
